...
const server = http.createServer(async (req, res) => {
  ...
  if (!resumeText.trim()) {
    return sendJson(res, 400, { error: "resumeText is required" });
  }
  if (!role.trim()) {
  ...
  if (!apiKey) {
    return sendJson(res, 500, { error: "ANTHROPIC_API_KEY is not set in environment" });
  }
  ...
  try {
    ...
    return sendJson(res, 200, { result: resultText });
  } catch (e) {
    const status = e.status && Number.isInteger(e.status) ? e.status : 500;
    return sendJson(res, status, { error: e.message || "Analysis failed" });
  }
});
